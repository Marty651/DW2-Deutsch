...
# TODO: Neue Version
function ProcessOneNeu($ElementEnNew, $ElementEnOld, $ElementDe, $RuleSetName, $FileName, $Rule)
{
...
if ($true) {
    foreach ($ruleSet in $RuleSets) {
        ...
        foreach ($fileName in $fileNames) {
            ...
            Write-Debug ">>> ID-Regel-Tag: $idRuleTag | IDs: $($ids -join ", ")"
                ...
                foreach ($innerRule in $rules) {
                    ...
                    try {
                        if ($elementsEnNewCount -eq 1) {
                            ...
                            # Must be checked!
                            ProcessOneNeu  -ElementEnNew $elementsEnNew -ElementEnOld $elementsEnOld -ElementDe $elementsDe `
                                        -RuleSetName $ruleSetName -FileName $fileName -Rule $rule 
                            continue
                        }
                        ...
                        for ($i = 0; $i -lt $elementsEnNewCount; $i++) {
                            $ruleInfo = "$rule ($i)"
                            ProcessOneNeu  -ElementEnNew $elementsEnNew[$i] -ElementEnOld $elementsEnOld[$i] -ElementDe $elementsDe[$i] `
                                        -RuleSetName $ruleSetName -FileName $fileName -Rule "$rule ($i)"
                        }   
                    }
                    catch {
